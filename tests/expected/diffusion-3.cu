  ...
  for(int i_ele = THREAD_ID; i_ele < n_ele; (i_ele += THREAD_COUNT))
  {
    ...
    for(int i_r_0 = 0; i_r_0 < 3; i_r_0++)
    {
      ...
      for(int i_g = 0; i_g < 6; i_g++)
      {
        ...
        for(int i_d_0 = 0; i_d_0 < 2; i_d_0++)
        {
          localTensor[(i_ele + (n_ele * i_r_0))] += ((0.5 * ((d_CG1[(((i_ele + (n_ele * i_d_0)) + (2 * (n_ele * i_g))) + (6 * (2 * (n_ele * i_r_0))))] * d_CG1[(((i_ele + (n_ele * i_d_0)) + (2 * (n_ele * i_g))) + (6 * (2 * (n_ele * i_r_0))))]) * (0.001 * (-1 * dt)))) * detwei[(i_ele + (n_ele * i_g))]);
        };
      };
    };
  };
